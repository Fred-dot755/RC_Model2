...
// 在 visual.c 的顶部修改变量定义：
// 强制让这两个 DMA 缓冲区起始地址 32 字节对齐
#if defined(__ICCARM__) // IAR
#pragma data_alignment=32
ReceiveData_t visual_data;
#pragma data_alignment=32
SendData_t visual_send_data;
#elif defined(__CC_ARM) || defined(__GNUC__) // Keil MDK or GCC
__attribute__((aligned(32))) ReceiveData_t visual_data;
__attribute__((aligned(32))) SendData_t visual_send_data;
#else
ReceiveData_t visual_data;
SendData_t visual_send_data;
#endif
...
bool check_crc16(const uint8_t * data, uint32_t len)
{
  ...
  // 按照上位机的小端逻辑提取末尾两个字节
  uint16_t crc16 = (uint16_t)((data[len - 1] << 8) | data[len - 2]);
  return get_crc16(data, len - 2) == crc16;
}


static uint8_t  rx_buffer[MEIC_PACKET_SIZE];
static uint16_t rx_index = 0;


bool meic_protocol_parse_packet(const uint8_t *packet, uint16_t len, ReceiveData_t *out_data)
{
    // 如果传入的数据长度还不如一个结构体大，直接返回
    if (len < MEIC_PACKET_SIZE) {
        return false;
    }

    // 遍历缓冲区寻找帧头 "MEIC"
    // 注意边界条件：i <= len - MEIC_PACKET_SIZE，确保剩下的数据足够一帧的长度
    for (uint16_t i = 0; i <= len - MEIC_PACKET_SIZE; i++) 
    {
        if (packet[i] == 'M' && packet[i+1] == 'E' && 
            packet[i+2] == 'I' && packet[i+3] == 'C') 
        {
            // 找到帧头，对从此处开始的 MEIC_PACKET_SIZE 长度的数据进行 CRC 校验
            if (check_crc16(&packet[i], MEIC_PACKET_SIZE)) {
                
                // 校验通过，拷贝到输出结构体中
                memcpy(out_data, &packet[i], MEIC_PACKET_SIZE);
                return true; 
            }
            // 如果校验失败，循环会继续往后寻找下一个可能的帧头
        }
    }
...
void meic_protocol_send_packet_dma(UART_HandleTypeDef *huart, int pitch ,int yaw ,int work_mode)
{
    ...
    // 2. 填充帧头
    visual_send_data.head[0] = 'M';
    visual_send_data.head[1] = 'E';
    visual_send_data.head[2] = 'I';
    ...
    uint16_t crc_result = get_crc16((const uint8_t *)&visual_send_data, calc_len);
