...
void unitree_init(void)
{
    ...
    {
        memset(&unitree_cmd[i], 0, sizeof(UnitreeMotorCmd_t));
        memset(&unitree_data[i], 0, sizeof(UnitreeMotorData_t));
        unitree_cmd[i].id = i;
        unitree_cmd[i].mode = 0; // 初始为锁定模式
        unitree_angle_init[i] = 0.0f;
        unitree_flag[i] = 0;
    }
...
/**
 * @brief 数据转换：将用户命令转换为协议数据包
 * @param motor_s 电机命令结构体指针
 */
void unitree_modify_data(UnitreeMotorCmd_t *motor_s)
{
    ...
    motor_s->motor_send_data.comd.pos_des  = ((motor_s->Pos/(180/3.1416)) + unitree_angle_init[motor_s->id]) / 6.2832 * 32768.0 * 6.33 ;
    motor_s->motor_send_data.comd.spd_des = motor_s->W / 6.28318f * 256.0f;
    motor_s->motor_send_data.comd.tor_des = motor_s->T * 256.0f;
    motor_s->motor_send_data.CRC16 = unitree_crc_ccitt(0, (uint8_t *)&motor_s->motor_send_data, sizeof(RIS_ControlData_t) - sizeof(motor_s->motor_send_data.CRC16));
}

/**
 * @brief 数据解析：将协议数据包转换为用户数据
 * @param motor_r 电机反馈结构体指针
 */
void unitree_extract_data(UnitreeMotorData_t *motor_r)
{
    ...
    motor_r->calc_crc = unitree_crc_ccitt(0, (uint8_t *)&motor_r->motor_recv_data, 
                                         sizeof(RIS_MotorData_t) - sizeof(motor_r->motor_recv_data.CRC16));
    ...
    else
    {
        ...
        motor_r->Pos =  6.2832f * ((float)motor_r->motor_recv_data.fbk.pos) / 32768.0f / 6.33f;
        motor_r->pos = (motor_r->Pos - unitree_angle_init[motor_r->motor_id])*(180.0/3.1416);
        motor_r->footForce = motor_r->motor_recv_data.fbk.force;
        motor_r->correct = 1;

        // 记录初始位置
        if (unitree_flag[motor_r->motor_id] == 0)
        {
...
/**
 * @brief 发送命令到指定电机
 * @param motor_id 电机ID (0-UNITREE_MOTOR_NUM-1)
 * @return HAL状态
 */
HAL_StatusTypeDef unitree_send_command(uint8_t motor_id)
{
    if (motor_id >= UNITREE_MOTOR_NUM)
        return HAL_ERROR;
        
    unitree_modify_data(&unitree_cmd[motor_id]);
    
    // 使用USART2发送数据（硬件RS485自动控制方向）
    return HAL_UART_Transmit(&huart2, (uint8_t *)&unitree_cmd[motor_id].motor_send_data, 
                            sizeof(unitree_cmd[motor_id].motor_send_data), 100);
}
...
HAL_StatusTypeDef unitree_communicate(uint8_t motor_id)
{
    ...
    // 发送命令
    status = unitree_send_command(motor_id);
    if (status != HAL_OK)
        return status;
    ...
    // 接收反馈
    status = unitree_receive_feedback(motor_id);
    if (status != HAL_OK)
        return status;
...
// CRC-CCITT查表
const uint16_t unitree_crc_ccitt_table_U[256] = {
...
/**
 * @brief CRC-CCITT单字节计算
 */
uint16_t unitree_crc_ccitt_byte(uint16_t crc, const uint8_t c)
{
    return (crc >> 8) ^ unitree_crc_ccitt_table_U[(crc ^ c) & 0xff];
}
...
{
    while (len--)
        crc = unitree_crc_ccitt_byte(crc, *buffer++);
    return crc;
}