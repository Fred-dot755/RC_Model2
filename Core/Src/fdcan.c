...
void fdcan1_filter_init(void)
{
    ...
    // Configure filter for standard ID
    ...
    fdcan_filter_st.FilterID1 = 0x000;
    fdcan_filter_st.FilterID2 = 0x000;
    fdcan_filter_st.RxBufferIndex = 0;
    
    if (HAL_FDCAN_ConfigFilter(&hfdcan1, &fdcan_filter_st) != HAL_OK)
    {
        Error_Handler();
    }
    
    // Configure global filter: reject non-matching frames, accept remote frames
...
void fdcan2_filter_init(void)
{
    ...
    if (HAL_FDCAN_ConfigFilter(&hfdcan2, &fdcan_filter_st) != HAL_OK)
    {
        Error_Handler();
    }
...
void fdcan3_filter_init(void)
{
    ...
    if (HAL_FDCAN_ConfigFilter(&hfdcan3, &fdcan_filter_st) != HAL_OK)
    {
        Error_Handler();
    }
...
// RX Callback function for FDCAN - 分离处理CAN1和CAN2
